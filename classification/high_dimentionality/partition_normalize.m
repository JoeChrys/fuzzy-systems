...
function [trnData,chkData,tstData] = partition_normalize(data)
    ...
    % Anonymous function to normalize variables
    ...
    class3Count = sum(data(:,end) == 3);
    class4Count = sum(data(:,end) == 4);
    class5Count = sum(data(:,end) == 5);
    
    % Partition data according to class
    class1Data = data(data(:,end) == 1,:);
    class2Data = data(data(:,end) == 2,:);
    class3Data = data(data(:,end) == 3,:);
    class4Data = data(data(:,end) == 4,:);
    class5Data = data(data(:,end) == 5,:);
    ...
    trnData = [class1Data(1:round(class1Count*0.6), :); ...
        ...
        class3Data(1:round(class3Count*0.6), :); ...
        class4Data(1:round(class4Count*0.6), :); ...
        class5Data(1:round(class5Count*0.6), :) ];
    chkData = [class1Data(round(class1Count*0.6)+1:round(class1Count*0.8), :); ...
        class2Data(round(class2Count*0.6)+1:round(class2Count*0.8), :); ...
        class3Data(round(class3Count*0.6)+1:round(class3Count*0.8), :); ...
        class4Data(round(class4Count*0.6)+1:round(class4Count*0.8), :); ...
        class5Data(round(class5Count*0.6)+1:round(class5Count*0.8), :) ];
    tstData = [class1Data(round(class1Count*0.8)+1:end, :); ...
        class2Data(round(class2Count*0.8)+1:end, :); ...
        class3Data(round(class3Count*0.8)+1:end, :); ...
        class4Data(round(class4Count*0.8)+1:end, :); ...
        class5Data(round(class5Count*0.8)+1:end, :) ];

    % Add max-min values to prevent evaluation warnings
    [maxVal, maxIdx] = max(data(:,1:end-1));
    [minVal, minIdx] = min(data(:,1:end-1));
    for i=1:length(maxIdx)
        trnData = [trnData; data(maxIdx(i),:)];
        chkData(chkData(:,i)==maxVal(i),:) = [];
        tstData(tstData(:,i)==maxVal(i),:) = []; 
    end
    for i=1:length(minIdx)
        trnData = [trnData; data(minIdx(i),:)];
        chkData(chkData(:,i)==minVal(i),:) = [];
        tstData(tstData(:,i)==minVal(i),:) = [];
    end
